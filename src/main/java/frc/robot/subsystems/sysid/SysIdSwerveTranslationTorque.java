 ...
 * drivetrain.
 * Thanks to joejazz from FRC 5712!
 */
public class SysIdSwerveTranslationTorque implements SwerveRequest {
  ...
  public StatusCode apply(SwerveControlParameters parameters, SwerveModule<?, ?, ?>... modulesToApply) {
    for (int i = 0; i < modulesToApply.length; ++i) {
      modulesToApply[i].apply(m_driveRequest.withOutput(TorqueCurrentToApply), m_steerRequest.withPosition(0));
    }
   ...
   * @return this request
   */
  public SysIdSwerveTranslationTorque withTorqueCurrent(double current) {
   ...
   * @return this request
   */
  public SysIdSwerveTranslationTorque withTorqueCurrent(Current current) {
