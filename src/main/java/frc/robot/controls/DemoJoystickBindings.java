...
import static frc.robot.Constants.TeleopDriveConstants.MAX_DEMO_ANGULAR_VELOCITY;
import static frc.robot.Constants.TeleopDriveConstants.MAX_DEMO_VELOCITY;
...
public class DemoJoystickBindings extends ControlBindings {
  ...
  public Supplier<LinearVelocity> translationX() {
    return () -> translationX
        .mut_replace(MAX_DEMO_VELOCITY.in(MetersPerSecond) * -squareAxis(leftJoystick.getY()), MetersPerSecond);
  }

  @Override
  public Supplier<LinearVelocity> translationY() {
    return () -> translationY
        .mut_replace(MAX_DEMO_VELOCITY.in(MetersPerSecond) * -squareAxis(leftJoystick.getX()), MetersPerSecond);
  }

  @Override
  public Supplier<AngularVelocity> omega() {
    return () -> omega.mut_replace(
        MAX_DEMO_ANGULAR_VELOCITY.in(RadiansPerSecond) * -squareAxis(rightJoystick.getX()),
          RadiansPerSecond);
  }
